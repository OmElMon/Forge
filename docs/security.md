...
Before production: add rate limiting, account lockout, password reset, email verification, MFA for privileged roles, CSRF tokens for future mutation routes, strict security headers, S3 upload scanning, database row-level-security policies, and centralized security event monitoring.
