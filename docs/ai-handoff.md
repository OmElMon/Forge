...
3. Real messaging adapter wiring — the user activates a provider account and sets `messaging_provider=...`; the port and delivery path are ready, and the follow-up policy surface can double as the settings UI for toggling rules.
4. Error reporting wiring — implement the `docs/operations.md` plan (sentry-sdk + @sentry/nextjs behind an empty-DSN no-op) once a DSN exists; everything else in the plan is ready to follow.
