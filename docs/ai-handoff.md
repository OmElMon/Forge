...
- Alembic migrations through `20260901_0022`.
...
- Invoices page with estimate/invoice workflow — all transitions (send, approve, convert-to-invoice, mark-paid, void, reopen) go through the dedicated action endpoints at `app/api/invoices/[id]/[action]`; the edit form only changes metadata, so status changes stay audited workflow transitions.
...
- None in the invoice workflow: send/approve/convert/mark-paid/void/reopen all flow through dedicated endpoints (`POST /invoices/{id}/{action}`), so `PATCH /invoices/{id}` is reserved for metadata edits (title, customer, due date, notes) rather than status transitions.
...
8. Durable shared rate limiting — done: `rate_limiter_backend` (`memory` | `redis`) selects a Redis-backed `RedisFixedWindowLimiter` for the shared store (atomic INCR against window-slot keys, EXPIRE, same 429 + `Retry-After` contract, fails open if the store errors so throttling never downs the API). Default stays in-process `memory`; flip `RATE_LIMITER_BACKEND=redis` when the API runs more than one instance. The load/concurrency half of Level 5 is done (item 14); worker-queue capacity verification remains for a real Celery workload.
9. Customer CSV import — done: `POST /customers/import` (multipart UTF-8 CSV) + `GET /customers/import/template`; row-level validation with a per-row error report, BOM/unknown-column tolerance, in-file duplicate email/phone skipping, per-customer `customer.created` events + one `customer.import` audit entry, all tenant-scoped. The Customers dashboard page has an Import card (file picker, template download, per-row error list, list refresh after import).
10. Finer per-slice RBAC — done: mutations are gated by role groups defined in `app/api/deps.py` (`BACK_OFFICE_ROLES` = owner/admin/office_staff for customers + addresses + equipment + CSV import, intake create/update/convert, invoices + line items + lifecycle steps, follow-up resolution; `OPERATIONS_ROLES` = owner/admin/dispatcher/office_staff for jobs create/update/schedule/assign/start/complete/cancel and technician create/update; `CONFIG_ROLES` = owner/admin for follow-up procedure rules; company profile already owner/admin). Reads stay open to every authenticated role; technicians are read-only in the dashboard. Backed by `tests/test_rbac.py`.
11. Login lockout + security headers — done: failed logins are tracked per account email and per client IP (`app/core/lockout.py`, mirroring the rate-limiter backends and fail-open posture); past `account_lockout_max_attempts` failures in the window, login returns a generic 429 + `Retry-After` for `account_lockout_duration_seconds` that doesn't reveal whether an email exists, and a successful login resets the counters. `security_headers_middleware` (outermost) adds `nosniff`, `DENY`, referrer policy, permissions policy, COOP, and HSTS to every response, including rate-limited and 4xx paths. Tests in `tests/test_lockout.py` + `tests/test_security_headers.py`. Post-login hardening still open: email verification, MFA for privileged roles, and CSP for the web host (see `docs/security.md`).
12. Email verification — done (backend): `users.email_verified`/`email_verified_at` (migration `20260830_0020`), single-use codes via `POST /auth/email-verify` (request) and `/auth/email-verify/confirm` (redeem), delivered through the messaging port with an in-UI dev code when the provider is `disabled`/`recording`, re-request expiring outstanding codes, `auth.email_verified` audit entries, and `email_verified` surfaced on `/auth/me`. Enforcement is a config flip (`EMAIL_VERIFICATION_REQUIRED`, default off) enforced in `authenticate` with a 403. Tests in `tests/test_email_verification.py`. A `verify-email` page in the web app is the follow-up, plus flipping the setting on pre-launch.
13. TOTP MFA — done (backend): owner/admin-only enrollment (migration `20260830_0021` + `mfa_settings` table, pyotp^2 dependency), `POST /auth/mfa/enroll` returns Base32 secret + `otpauth://` URI + 8 recovery codes stored BCrypt-hashed, `POST /auth/mfa/enroll/confirm` activates only after a live code proves the scan, `POST /auth/mfa/disable` re-proves identity first. Once confirmed, `authenticate` returns a 5-minute `mfa_session` challenge instead of tokens (`/auth/login` response is now `LoginResponse` tagged by `mfa_required`); `POST /auth/mfa/verify` redeems a TOTP or single-use recovery code for the real token pair, consuming recovery codes and emitting `auth.mfa.*` audit events. Suspended-non-owner checks repeat at verify time so a suspended workspace can't bypass. `GET /auth/mfa` reports enrollment state for the settings UI. Tests in `tests/test_mfa.py`. Web parity is done too: `/login` swaps to an MFA code step when `mfa_required` is returned (verify via `/api/auth/mfa/verify`, which sets the session cookies), and the login challenge survives a hard refresh via a short `sessionStorage` hold. Reconfiguration is now pending-safe (migration `20260901_0022`): `enroll` while confirmed stages the new secret as `pending_secret`/`pending_recovery_hashes` and keeps the old secret active until the new one is confirmed, so canceling or failing a reconfigure never silently disables MFA; the Settings `Two-factor authentication` card (`apps/web/components/mfa-section.tsx`) asks for a live code before showing the new recovery codes. Only pre-launch hooks float: `EMAIL_VERIFICATION_REQUIRED` and real messaging.

14. Load/concurrency harness — done: `apps/api/scripts/load_test.py` drives a running API with concurrent register + login + read storms and exits non-zero on any HTTP 5xx, any transport error, p95 latency over `--max-p95-ms`, or a locked-account probe that fails to 429 (login defense never fired). Pure logic is unit-tested in `tests/test_load_harness.py` (14 tests); the async runner runs against a real stack. The CI `concurrency` job bootstraps Postgres + Redis service containers, applies `alembic upgrade head` to a real engine, boots uvicorn, and runs the harness (`--concurrency 8 --duration 12`) — the only job that exercises async DB sessions, the rate limiter, and lockout end-to-end. Run it locally via `docker compose up -d postgres redis api` then `.venv/bin/python scripts/load_test.py`; see `docs/operations.md` §3. Remaining Level 5 scaling item: worker-queue capacity verification (Celery) once a real provider moves production traffic.

15. Dedicated invoice void/reopen endpoints — done: `POST /invoices/{id}/void` (draft/sent/approved estimates or invoices only, `invoice.voided` event + audit) and `POST /invoices/{id}/reopen` (`{status: draft|sent}` validated per source: void→draft/sent, converted→draft, paid→sent; `invoice.reopened` event + audit). `PATCH /invoices/{id}` no longer carries void/reopen in the web UI; the revenue page drives every transition through `app/api/invoices/[id]/[action]` and the edit form hides void/converted/paid states behind the workflow buttons.
