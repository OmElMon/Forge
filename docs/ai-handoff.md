...
1. More automation rule depth (policy-managed rules, self-resolution when an invoice is sent for a completed job) and real messaging adapter wiring — the user activates a provider account and sets `messaging_provider=...`; the port and delivery path are ready.
